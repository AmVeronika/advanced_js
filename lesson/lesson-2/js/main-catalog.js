...
const renderProductCatalog = (item) => {
   return `<div class="product__card shadow filter-card trans">
      ...
      <a href="product.html" class="product-card__name">
         <h5 class="product-card__h5">${item.title}</h5>
         <p class="product-card__text">${item.descp}</p>
         <p class="product-card__text-options">$${item.price}</p>
      </a>
...
//________ C A R T _____________
//______________________________



// Массив объектов (карточек продуктов в корзине)
const productsCart = [
...
// 3. Функция для формирования вёрстки каждого товара в корзине
const renderProductCart = (item) => {
   return `<div class="shopping__card shadow">
   <a href="product.html" class="shopping-card__link">
      <img class="shopping-card__img  scale trans" src="img/cart/shopping-card${item.id}.jpg" width="262"
         height="100%" alt="product">
   </a>
   <div class="shopping-card__name">
      <h3 class="shopping-name__h3">${item.title}</h3>
      <p class="shopping-card__text">Price: <span class="shopping-card__text-color">$${item.price}</span></p>
      <p class="shopping-card__text">Color: <span class="shopping-card__text-span">${item.color}</span></p>
      <p class="shopping-card__text">Size: <span class="shopping-card__text-span">${item.size}</span></p>
...
// 2.
const renderPageCart = (list) => {
   document.querySelector('.shopping__grid-wrap').innerHTML = list.map((item) => renderProductCart(item)).join(' '); //Использовали метод .join() для разделения элементов массива пробелами
}



// 1. Запуск функции для добавления товаров в блок
renderPageCart(productsCart);

//________ C A R T __ E N D ____
//______________________________