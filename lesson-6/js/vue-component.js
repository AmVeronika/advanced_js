...
Vue.component("catalog-card", {
   ...
   template: ` <article class="product__grid-wrap product__grid-wrap--catalog">
                  <!--Card Product Vue-->
                  <div class="product__card shadow filter-card trans" v-for="product in filterproducts">
                     <!-- Создаём карточки из массива фильтра 'filterProducts' (в который карточки попадают из основного массива json)-->
                     <img :src="'img/catalog/product-card'+product.id_product+'.jpg'" width="360" height="420" alt="product card trans" class="product-card__img">
                     <a href="product.html" class="product-card__name">
                        <h5 class="product-card__h5">{{product.title}}</h5>
                        <p class="product-card__text">{{product.descp}}</p>
                        <p class="product-card__text-options">$ {{product.price}}</p>
                     </a>
                     <button class="card-hover button-hover4 trans1" v-on:click="{{$root.addListProductCart(product)}}">
...
// ------------------- CART CONTAINER -----------------------
// ----------------------------------------------------------
Vue.component("cart-wrapp", {
   props: ['cart'],
   template: `       <div class="shopping__wrap">
                        <!-- Cards container  
                       <!-- <p style="font-weight: 600; font-size: 36px; margin: 253px auto 0;" v-if="!cart.length">Вы ничего ещё не заказали</p> -->
                      <cart-card :cart="cart"></cart-card>
                     </div>`
});
Vue.component("cart-card", {
   ...
   template: ` <div class="shopping__grid-wrap">
                  ...
                  <div class="shopping__card shadow" v-for="cartProduct in cart">
                     ...
                     :src="'img/catalog/product-card' +cartProduct.id_product+ '.jpg'" width="262" height="100%" alt="product">
                     </a>
                  ...
                  <button type="submit" class="shopping-card__close"
                     v-on:click="{{$root.removeListProductCart(cartProduct)}}">
